...
NN_MODEL = './results/nn_model_ep_3600.ckpt'
#NN_MODEL = None
#NN_MODEL = './results/pretrain_linear_reward.ckpt'
def testing(epoch, nn_model, log_file):
    # clean up the test results folder
    os.system('rmdir /s /q ' + TEST_LOG_FOLDER)
    ...
    log_file.write(log_line1.encode())
...
def central_agent(net_params_queues, exp_queues):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_test', 'wb') as test_log_file:
        ...
        # # 保存网络模型
        # save_path = saver.save(sess, SUMMARY_DIR + "/second"
        #                           + ".ckpt")
        # logging.info("Model saved in file: " + save_path)
        # testing(epoch,
        #      SUMMARY_DIR + "/second" + ".ckpt",
        #      test_log_file)
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            log_file.write(bytes(str(time_stamp),'utf-8') + b'\t' +
                           bytes(str(VIDEO_BIT_RATE[bit_rate]), 'utf-8') + b'\t' +
                           bytes(str(buffer_size), 'utf-8') + b'\t' +
                           bytes(str(rebuf), 'utf-8') + b'\t' +
                           bytes(str(video_chunk_size), 'utf-8') + b'\t' +
                           bytes(str(delay), 'utf-8') + b'\t' +
                           bytes(str(reward), 'utf-8') + b'\n')
            log_file.flush()
